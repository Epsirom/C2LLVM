...
LLVMGenerator.prototype.handleCONDITION = function(node) {
    ...
    if(node.children[childindex] != undefined) {
        if(node.children[childindex].token.type == lexer.MEMBERVAR) {
            var firstv = this.loadVARIABLE([node.children[0], node.children[1]]);
            childindex ++;
        } else {
            var firstv = this.loadVARIABLE([node.children[0]]);
        }
        firstllvm =  firstv.type + ' %' + firstv.llvmname;
        var condllvm = this.newPmap();
        if(node.children[childindex].token.type == lexer.CONSTNUM) {
            secondllvm = node.children[childindex].token.text;
        }
        this.body.push('%' + condllvm + ' = ' + 'icmp ' + cmpOp + ' ' + firstllvm + ', ' + secondllvm);
    } else {
        var  v = this.loadVARIABLE(node.children[0]);
        var condllvm = this.newPmap();
...
LLVMGenerator.prototype.handleIFSTAT = function(node) {
    ...
    var condv = this.handleCONDITION(node.children[0].children[0]);
    var brCode = 'br i1 ' + condv.llvmname + ', label %if.then, ';
    var hasElse = false;
