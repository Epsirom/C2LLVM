...
LLVMGenerator.prototype.newPmap = function() {
    ...
    return '%rr' + this.pmapcount;
};

LLVMGenerator.prototype.resetPmap = function() {
    for (var i = 0, len = this.pmapcount; i < len; ++i) {
        this.data.pmap[i] = false;
    }
    this.pmapcount = 0;
};
...
LLVMGenerator.prototype.handleASSIGN = function(node) {
    ...
    if (rightnode.children[0].token.type == lexer.VARIABLE) {
        ...
        if (rightllvm[0] == '%' && !vr.fulltype) {
            rightllvm = this.newPmap();
            this.body.push(rightllvm + ' = load ' + righttype + '* ' + vr.llvmname);
        }

        if (rightllvm[0] == '%' && lefttype != righttype) {
            rightllvm = this.newPmap();
            this.body.push(rightllvm + ' = bitcast ' + righttype + ' ' + vr.llvmname + ' to ' + lefttype);
        } else if (rightllvm[0] != '%' && lefttype[lefttype.length - 1] == '*') {
            rightllvm = 'null';
        }

        this.body.push('store ' + lefttype + ' ' + rightllvm + ', ' + lefttype + '* ' + leftllvm);
        ...
        var si = this.handleSHIFT_EXPR(rightnode.children[0]);
        righttype = si.type;
        rightllvm = si.llvmname;
        if (!si.fulltype) {
            rightllvm = this.newPmap();
            this.body.push(rightllvm + ' = load ' + si.type + '* ' + si.llvmname);
        }
        this.body.push('store ' + righttype + ' ' + rightllvm + ', ' + lefttype + '* ' + leftllvm);
    }
    return {
...
LLVMGenerator.prototype.loadVARIABLE = function(nodes) {
    if (nodes.length == 1) {
        ...
        } else if (nodes[0].token.type == lexer.ASSIGNMENT) {
            return this.handleASSIGN(nodes[0]);
        }
...
LLVMGenerator.prototype.handleSHIFT_EXPR = function(node) {
    ...
    if (s1llvm[0] == '%') {
        s1llvm = this.newPmap();
        this.body.push(s1llvm + ' = load ' + s1.type + '* ' + s1.llvmname);
    }
    if (s2llvm[0] == '%') {
        s2llvm = this.newPmap();
        this.body.push(s2llvm + ' = load ' + s2.type + '* ' + s2.llvmname);
    }
...
LLVMGenerator.prototype.handleFUNC_INVOKE = function(node) {
    ...
    var rtntype;
    if (func == 'malloc') {
        var vv = this.loadVARIABLE(node.children[1].children[0].children);
        if (vv.llvmname[0] != '%') {
            this.body.push(llvmname + ' = call i8* @malloc(i32 ' + vv.llvmname + ')');
        } else {
            var tllvm = this.newPmap();
            this.body.push(tllvm + ' = load ' + vv.type + '* ' + vv.llvmname);
            this.body.push(llvmname + ' = call i8* @malloc(i32 ' + tllvm + ')');
        }
        rtntype = 'i8*';
    } else if (func == 'memcpy') {
        this.body.push('call void @llvm.memcpy.p0i8.p0i8.i32(' + this.loadFUNC_PARAMS(node.children, ['i8*', 'i8*', 'i32']) + ', i32 1, i1 false)')
        rtntype = 'void';
    } else if (func == 'free') {
        var v = this.loadVARIABLE(node.children[1].children[0].children);
        var ld = this.newPmap();
        this.body.push(ld + ' = load ' + v.type + '* ' + v.llvmname);
        var cst = this.newPmap();
        this.body.push(cst + ' = bitcast ' + v.type + ' ' + ld + ' to i8*');
        this.body.push('call void @free(i8* ' + cst + ')');
        rtntype = 'void';
    } else if (func == 'printf') {
        ...
        this.body.push((rtntype == 'void' ? '' : (llvmname + ' = ')) + 'call ' + rtntype + ' @' + func + '(' + this.loadFUNC_PARAMS(node.children) + ')');
    }
    return {
        type: rtntype,
        llvmname: llvmname
...
LLVMGenerator.prototype.loadFUNC_PARAMS = function(nodes, ptypes) {
    var rtn = '';var etype, ollvm;
        ...
        switch (nodes[i].children[0].token.type) {
            ...
            case lexer.VARIABLE:
                ...
                if (ptypes) {
                    ...
                    if (v.type != etype) {
                        llvm = this.newPmap();
                        this.body.push(llvm + ' = bitcast ' + v.type + ' ' + ollvm + ' to ' + etype);
                    }
                }
                rtn += (etype + ' ' + llvm);
                break;
            ...
            case lexer.FUNC_INVOKE:
                ...
                if (ptypes) {
                    ...
                    if (fi.type != etype) {
                        llvm = this.newPmap();
                        this.body.push(llvm + ' = bitcast ' + fi.type + ' ' + ollvm + ' to ' + etype);
                    }
                }
                rtn += (etype + ' ' + fi.llvmname);
                break;
            ...
            case lexer.CASTTYPE:
                ...
                if (ptypes) {
                    ...
                    if (fi2.type != etype) {
                        llvm = this.newPmap();
                        this.body.push(llvm + ' = bitcast ' + fi2.type + ' ' + ollvm + ' to ' + etype);
                    }
                }
...
LLVMGenerator.prototype.handlePREFIX_PLUS = function(node) {
    if (node.children[0].token.type == lexer.VARIABLE) {
...
LLVMGenerator.prototype.handleCONDITION = function(node) {
    ...
    if(unaryFlag) {
        ...
        this.body.push(condllvm + ' = ' + 'icmp ne ' + v.type + ' '+ v.llvmname + ', ' + (v.type[v.type.length - 1] == '*' ? 'null' : '0'));
    } else {
        var v1, v2;
        var cmpCode = '';
        var cmptype, c1llvm, c2llvm;
        v1 = this.handleSHIFT_EXPR(node.children[0]);
        cmptype = v1.type;
        c1llvm = v1.llvmname;
        if (!v1.fulltype) {
            c1llvm = this.newPmap();
            this.body.push(c1llvm + ' = load ' + cmptype + '* ' + v1.llvmname);
        }
        v2 = this.handleSHIFT_EXPR(node.children[1]);
        c2llvm = v2.llvmname;
        if (!v2.fulltype) {
            c2llvm = this.newPmap();
            this.body.push(c2llvm + ' = load ' + v2.type + '* ' + v2.llvmname);
            if (v2.type != cmptype) {
                var tllvm = this.newPmap();
                this.body.push(tllvm + ' = bitcast ' + v2.type + ' ' + c2llvm + ' to ' + cmptype);
                c2llvm = tllvm;
            }
        } else if (cmptype[cmptype.length - 1] == '*') {
            c2llvm = 'null';
        }
        condllvm = this.newPmap();
        this.body.push(condllvm + ' = icmp ' + cmpOp + ' ' + cmptype + ' ' + c1llvm + ', ' + c2llvm);
    }
    return condllvm;
};
...
LLVMGenerator.prototype.handleWHILESTAT = function(node) {
    ...
    this.body.push('br i1 ' + condv + ', label %while' + labelnum + '.body, label %while' + labelnum + '.end');
