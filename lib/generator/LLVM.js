...
LLVMGenerator.prototype.handleASSIGN = function(node) {
    if (node.children.length == 1) {
        ...
        } else if (node.token.type == lexer.DECLARATOR) {
            return this.loadVARIABLE(onode.children[0].children);
    ...
    var vl = this.loadVARIABLE(leftnode.children);
    lefttype = vl.type;
    leftllvm = vl.llvmname;
    if (rightnode.children[0].token.type == lexer.DECLARATOR) {
        var vr = this.loadVARIABLE(rightnode.children);
        righttype = vr.type;
...
LLVMGenerator.prototype.loadVARIABLE = function(nodes) {
    if (nodes.length == 1) {
        ...
        var ii = this.handleSHIFT_EXPR(nodes[1].children[1].children);
...
LLVMGenerator.prototype.handleCASTTYPE = function(node) {
    ...
    switch (mb.token.type) {
        ...
        case lexer.DECLARATOR:
            var pr = this.getParam(mb.children[0].token.text);
            mbt = {
                type: this.get_full_type_id(pr),
                llvmname: pr.llvmname
            };
...
LLVMGenerator.prototype.handleSHIFT_EXPR = function(nodes) {
    switch (nodes[0].token.type) {
        case lexer.DECLARATOR:
            return this.loadVARIABLE(nodes);

        case lexer.CONSTNUM:
            return {
                type: 'i32',
                llvmname: nodes[0].token.text
            };
...
LLVMGenerator.prototype.handleARRAYINDEX = function(node) {
...
LLVMGenerator.prototype.handleFUNC_INVOKE = function(node) {
    ...
    if (func == 'malloc') {
        ...
        var v = this.loadVARIABLE(node.children[1]);
    ...
    for (var i = 1, len = nodes.length; i < len; ++i) {
        ...
        switch (nodes[i].children[0].token.type) {
            ...
            case lexer.DECLARATOR:
                ...
                var v = this.loadVARIABLE(nodes[i].children);
...
LLVMGenerator.prototype.handleCONDITION = function(node) {
    ...
    if(unaryFlag) {
        var v = this.loadVARIABLE(node.children);
        if(node.children.length == 1) {
            var newllvm = this.newPmap();
            this.body.push('%' + newllvm + ' = ' + 'load %' + v.type + v.llvmname);
            v.llvmname = newllvm;
        }
        var condllvm = this.newPmap();
        this.body.push('%' + condllvm + ' = ' + 'icmp ne ' + v.type + ' %'+ v.llvmname + ', 0');
    } else {
        var eleset = [[], []];
        var eleindex = -1;
        var elenum = node.children.length;
        for(var i = 0; i < elenum; i ++) {
            if(node.children[i].token.type == lexer.DECLARATOR || node.children[i].token.type == lexer.CONSTNUM)
                eleindex ++;
            eleset[eleindex].push(node.children[i]);
        }
        var v1, v2;
        var cmpCode = '';
        if(eleset[0][0].token.type == lexer.DECLARATOR)
            v1 = this.loadVARIABLE(eleset[0]);
        else if(eleset[0][0].token.type == lexer.CONSTNUM)
            v1 = {type:'constnum', llvmname: eleset[0][0].token.text};
        if(eleset[1][0].token.type == lexer.DECLARATOR)
            v2 = this.loadVARIABLE(eleset[1]);
        else if(eleset[1][0].token.type == lexer.CONSTNUM)
            v2 = {type:'constnum', llvmname: eleset[1][0].token.text};
        var condllvm = this.newPmap();
...
LLVMGenerator.prototype.handleIFSTAT = function(node) {
    ...
    var brCode = 'br i1 %' + condllvm + ', label %if.then, ';
    var hasElse = false;
    if(node.children[1] != undefined && node.children[1].token.type == lexer.ELSE_BRANCH) {
        hasElse = true;
    }
    if(hasElse)
        brCode += 'label %if.else';
    else
        brCode += 'label %if.end';
    this.body.push(brCode);
    this.body.push('if.then:');
    this.handleBLOCK(node.children[0].children[1]);
    this.body.push('br label %if.end');
    if(hasElse) {
        this.body.push('if.else:');
        this.handleBLOCK(node.children[1].children[0]);
        this.push('br label %if.end');
    }
    this.body.push('if.end:');
};

LLVMGenerator.prototype.handleFORSTAT = function(node) {
    this.handleASSIGN(node.children[0]);
    this.body.push('br label %for.cond');
    this.body.push('for.cond:');
    var condv = this.handleCONDITION(node.children[1]);
    this.push('br i1 %' + condv + ', label %for.body, label %for.end');
    this.push('for.body:');
    this.handleBLOCK(node.children[3]);
    this.push('br label %for.inc');
    this.push('for.inc:');
    this.handleASSIGN(node.children[2]);
    this.push('br label %for.cond');
    this.push('for.end:');
};

LLVMGenerator.prototype.handleWHILESTAT = function(node) {
    this.body.push('br label %while.cond');
    this.body.push('while.cond: ');
    var condv = this.handleCONDITION(node.children[0]);
    this.push('br i1 %' + condv.llvmname + ', label %while.body, label %while.end');
    this.body.push('while.body: ');
    this.handleBLOCK(node.children[1]);
    this.body.push('br label %while.cond');
    this.body.push('while.end: ');
};
