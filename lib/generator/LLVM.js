...
LLVMGenerator.prototype.handleASSIGN = function(node) {
    if (node.children[0].token.type == lexer.PREFIX_PLUS) {
        return this.handlePREFIX_PLUS(node.children[0]);
    } else if (node.children[0].token.type == lexer.PREFIX_MINUS) {
        return this.handlePREFIX_MINUS(node.children[0]);
    } else if (node.children[0].token.type == lexer.FUNC_INVOKE) {
        return this.handleFUNC_INVOKE(node.children[0]);
    }
    var lefttype;
    var leftllvm;
    var rightnode;
